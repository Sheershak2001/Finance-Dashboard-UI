const Summary = ({ title, amount, type, darkMode }) => {
  const config = {
    balance: {
  ...
  return (
    <div
      className={`${c.bg} border ${c.border} rounded-2xl p-5 hover:scale-[1.02] transition-all shadow-sm`}
    >
      <p
        className={`text-xs font-semibold tracking-widest uppercase ${c.label}`}
      >
        {title}
      </p>
